static inline void MeshUploadXformsScale(
...
{
  *out_count = 3;
  *out_bytes = 3 * 3 * sizeof(float);
  unsigned format = 0x8815; // GL_RGB32F
#ifdef _WIN32
...
static inline void MeshUploadXformsRotationQuaternion(
    char      *  in_path_to_binary,
    unsigned     in_buf_flags,
    ptrdiff_t *  out_count,
    ptrdiff_t *  out_bytes,
...
{
  *out_count = 3;
  *out_bytes = 3 * 4 * sizeof(float);
  unsigned format = 0x8814; // GL_RGBA32F
#ifdef _WIN32
  *out_fh = CreateFileA(in_path_to_binary, FILE_READ_DATA, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
  assert(*out_fh != INVALID_HANDLE_VALUE);
  *out_fm = CreateFileMappingFromApp(*out_fh, NULL, PAGE_READONLY, *out_bytes, NULL);
...
static inline void MeshUploadXformsTranslation(
    char      *  in_path_to_binary,
    unsigned     in_buf_flags,
    ptrdiff_t *  out_count,
    ptrdiff_t *  out_bytes,
...
static inline unsigned SimpleMeshUploadXformsScale(
    char     * in_path_to_binary,
    unsigned   in_buf_flags,
    unsigned * out_buf)
{
  ...
  unsigned buf = 0;
  unsigned tex = 0;
  MeshUploadXformsScale(in_path_to_binary, in_buf_flags, &count, &bytes, &fh, &fm, &map_ptr, &buf, &tex);
#ifdef _WIN32
...
static inline unsigned SimpleMeshUploadXformsRotationQuaternion(
    char     * in_path_to_binary,
    unsigned   in_buf_flags,
    unsigned * out_buf)
{
  ...
  unsigned buf = 0;
  unsigned tex = 0;
  MeshUploadXformsRotationQuaternion(in_path_to_binary, in_buf_flags, &count, &bytes, &fh, &fm, &map_ptr, &buf, &tex);
#ifdef _WIN32
  UnmapViewOfFile(map_ptr);
  CloseHandle(fm);
  CloseHandle(fh);
...
static inline unsigned SimpleMeshUploadXformsTranslation(
    char     * in_path_to_binary,
    unsigned   in_buf_flags,
    unsigned * out_buf)
{
  ...
  unsigned buf = 0;
  unsigned tex = 0;
  MeshUploadXformsTranslation(in_path_to_binary, in_buf_flags, &count, &bytes, &fh, &fm, &map_ptr, &buf, &tex);
#ifdef _WIN32
  UnmapViewOfFile(map_ptr);
  CloseHandle(fm);
  CloseHandle(fh);
